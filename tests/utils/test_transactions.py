...
def test_sub_tx_de_and_encoding():
    ...
    assert internal.toString() == internal_str
    ...
    assert token.toString() == token_str
    assert (
        token.toString(type_overwrite=SubTransactionType.ExternalTx)
        == "0x76f4263391a7d72f66cb1f254e8643e37ca739ab2859b9e9cd5b5bda3194332b"
    )
    assert (
        token.toString(type_overwrite=SubTransactionType.GenericLog)
