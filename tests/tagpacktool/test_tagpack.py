...
def test_simple_file_collection():
    prefix = "tests/tagpacktool/testfiles/simple"
...
def test_file_collection_with_yaml_include():
    bdir = "tests/tagpacktool/testfiles/yaml_inclusion"
...
def test_file_collection_with_multiple_yaml_include():
    bdir = "tests/tagpacktool/testfiles/yaml_inclusion_multiple"
    h_files = collect_tagpack_files(bdir)
...
def test_file_collection_with_missing_yaml_include_raises_exception():
    bdir = "tests/tagpacktool/testfiles/yaml_inclusion_missing_header"
    h_files = collect_tagpack_files(bdir)
...
def test_file_collection_simple_with_concepts(taxonomies):
    bdir = "tests/tagpacktool/testfiles/simple/with_concepts.yaml"
    h_files = collect_tagpack_files(bdir)
    headerfile_path, files = h_files.popitem()
    files = list(files)
...
def test_load_from_file_addr_tagpack(taxonomies):
    tagpack = TagPack.load_from_file(
        ...
        "tests/tagpacktool/testfiles/simple/ex_addr_tagpack.yaml",
        TagPackSchema(),
        taxonomies,
    )
...
def test_yaml_inclusion(taxonomies):
    tagpack = TagPack.load_from_file(
        "http://example.com/",
        "tests/tagpacktool/testfiles/yaml_inclusion/2021/01/20210101.yaml",
        TagPackSchema(),
        taxonomies,
        "tests/tagpacktool/testfiles/yaml_inclusion",
    )
    assert isinstance(tagpack, TagPack)
    assert "title" in tagpack.schema.header_fields
...
def test_yaml_inclusion_overwrite_abuse(taxonomies):
    tagpack = TagPack.load_from_file(
        "http://example.com/",
        "tests/tagpacktool/testfiles/yaml_inclusion/2021/02/20210201.yaml",
        TagPackSchema(),
        taxonomies,
        "tests/tagpacktool/testfiles/yaml_inclusion",
    )
...
#         "tests/tagpacktool/testfiles/simple/empty_tag_list.yaml",
#         TagPackSchema(),
#         taxonomies,
#     )
...
def test_multiple_tags_for_one_address_work(taxonomies):
    tagpack = TagPack.load_from_file(
        "http://example.com/packs",
        "tests/tagpacktool/testfiles/simple/multiple_tags_for_address.yaml",
        TagPackSchema(),
        taxonomies,
    )
    assert len(tagpack.tags) == 2
...
def test_duplicate_does_not_raise_only_inform(capsys, taxonomies):
    tagpack = TagPack.load_from_file(
        "http://example.com/packs",
        "tests/tagpacktool/testfiles/simple/duplicate_tag.yaml",
        TagPackSchema(),
        taxonomies,
    )
