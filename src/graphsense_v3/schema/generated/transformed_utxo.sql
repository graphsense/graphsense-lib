...
CREATE TABLE IF NOT EXISTS address_stats (
    ...
    cluster_address blob,                   -- UTXO: the cluster's root member
...
CREATE TABLE IF NOT EXISTS address_tx_pages (
    ...
    first_tx_id bigint,
    tx_page int,
    PRIMARY KEY ((address, is_outgoing), first_tx_id)
)
...
CREATE TABLE IF NOT EXISTS cluster_stats (
    cluster_address_bucket int,             -- murmur3(entity) % entity_buckets
    ...
    PRIMARY KEY (cluster_address_bucket, cluster_address, epoch)
)
    WITH CLUSTERING ORDER BY (cluster_address ASC, epoch ASC)
    AND compaction = {'class':'LeveledCompactionStrategy','sstable_size_in_mb':'160'};
...
CREATE TABLE IF NOT EXISTS cluster_transactions (
    ...
    tx_id bigint,
    value varint,
    PRIMARY KEY ((cluster_address, is_outgoing, tx_page), tx_id)
)
    WITH CLUSTERING ORDER BY (tx_id DESC)
    AND compaction = {'class':'SizeTieredCompactionStrategy'};
...
-- newest-first is the API default, the common request is served from the
-- tail alone and never touches a page.
CREATE TABLE IF NOT EXISTS cluster_transactions_recent (
    cluster_address blob,
    is_outgoing boolean,
    block_batch int,
    tx_id bigint,
    value varint,
    PRIMARY KEY ((cluster_address, is_outgoing, block_batch), tx_id)
)
    WITH CLUSTERING ORDER BY (tx_id DESC)
    AND compaction = {'class':'LeveledCompactionStrategy'}
    AND gc_grace_seconds = 259200;
...
-- USDT, all in one partition of about a megabyte. Read only when a range
-- filter is present.
CREATE TABLE IF NOT EXISTS cluster_tx_pages (
    cluster_address blob,
    is_outgoing boolean,
    first_tx_id bigint,
    tx_page int,
    PRIMARY KEY ((cluster_address, is_outgoing), first_tx_id)
)
    WITH CLUSTERING ORDER BY (first_tx_id DESC)
    AND compaction = {'class':'SizeTieredCompactionStrategy'};
...
CREATE TABLE IF NOT EXISTS cluster_incoming_relations (
    ...
    rel_bucket int,                         -- murmur3(far side) % relation_buckets
    src_cluster_address blob,
    epoch int,                              -- as address_stats: summable
    no_transactions bigint,                 -- was int
    value frozen<currency>,
    link_page_max int,                      -- epoch 0 only
    link_ordinal_next bigint,
    PRIMARY KEY ((dst_cluster_address, rel_bucket), src_cluster_address, epoch)
)
    WITH CLUSTERING ORDER BY (src_cluster_address ASC, epoch ASC)
    AND compaction = {'class':'LeveledCompactionStrategy','sstable_size_in_mb':'160'};
...
CREATE TABLE IF NOT EXISTS cluster_outgoing_relations (
    ...
    link_page_max int,                      -- epoch 0 only
    link_ordinal_next bigint,
    PRIMARY KEY ((src_cluster_address, rel_bucket), dst_cluster_address, epoch)
)
    WITH CLUSTERING ORDER BY (dst_cluster_address ASC, epoch ASC)
    AND compaction = {'class':'LeveledCompactionStrategy','sstable_size_in_mb':'160'};
...
-- materialise these tuples when computing no_transactions -- today they
-- aggregate them away, which is why /links has to rescan raw io membership.
CREATE TABLE IF NOT EXISTS cluster_link_transactions (
    src_cluster_address blob,
    dst_bucket int,                         -- murmur3(dst) % relation_buckets
    dst_cluster_address blob,
    tx_id bigint,
...
CREATE TABLE IF NOT EXISTS cluster_addresses (
    ...
    cluster_address blob,
    address blob,
    PRIMARY KEY ((cluster_bucket, cluster_address), address)
)
    WITH compaction = {'class':'SizeTieredCompactionStrategy'};
