...
from ..utils import batch, first_or_default, hex_to_bytearray, parse_timestamp
...
def ingest_async(
    ...
    def process(thrd_ctx, cmd, s, e, sink_config, data):
        print(cmd, s, e)
        if cmd == "load_blocks":
            blocks, txs = thrd_ctx.adapter.export_blocks_and_transactions(s, e)
            prepare_blocks_inplace(blocks, BLOCK_BUCKET_SIZE)
            return [
                (cmd, "store_block", s, e, blocks),
                (cmd, "load_receipts", s, e, txs),
            ]
        if cmd == "load_traces":
            traces = thrd_ctx.adapter.export_traces(s, e, True, True)
            prepare_traces_inplace(traces, BLOCK_BUCKET_SIZE)
            return [(cmd, "store_trace", s, e, traces)]
        elif cmd == "load_receipts":
            txs = data
            receipts, logs = thrd_ctx.adapter.export_receipts_and_logs(txs)
            enriched_txs = enrich_transactions(txs, receipts)
            prepare_logs_inplace(logs, BLOCK_BUCKET_SIZE)
            prepare_transactions_inplace(
                enriched_txs, TX_HASH_PREFIX_LEN, BLOCK_BUCKET_SIZE
            ...
            return [
                (cmd, "store_transaction", s, e, enriched_txs),
                (cmd, "store_log", s, e, logs),
            ]
        elif cmd.startswith("store_"):
            write_to_sinks(thrd_ctx.db, sink_config, cmd.replace("store_", ""), data)
            return []
        else:
            raise NotImplementedError(f"Command {cmd} not found.")
    ...
    with graceful_ctlc_shutdown() as check_shutdown_initialized:
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for block_ids in batch(range(start_block, end_block + 1, batch_size), n=3):
                ...
                remaining_tasks = [
                    ...
                    for cmd, (s, e) in product(["load_blocks", "load_traces"], ranges)
                ]

                while len(remaining_tasks) > 0:
                    completed = next(concurrent.futures.as_completed(remaining_tasks))
                    remaining_tasks.remove(completed)
                    for cmd, cont_task_name, s, e, data in completed.result():
                        if cmd == "load_blocks" and cont_task_name == "store_block":
                            blocks = data
    ...
    with graceful_ctlc_shutdown() as check_shutdown_initialized:
        for block_id in range(rounded_start_block, rounded_end_block + 1, batch_size):
            ...
            with suppress_log_level(logging.INFO):
                ...
                ):  # todo not necessary anymore, this case impossible
