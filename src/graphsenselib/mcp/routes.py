...
def make_component_fn(curation: CurationFile, paged_tools: set[str] | None = None):
    ...
    a `pagesize` query parameter are collected into it. The page-size transform
    and explicit-null middleware use that set. This is the only place the
    route's parameter list is visible, hence the out-parameter.
    """
