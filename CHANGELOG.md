...
- **Merged `/stats` entries declare the tags capability.** A backend entry carrying a `capabilities` list (the fine-grained lite declaration consumed by the dashboard) gets `"tags"` added during the merge: the backend cannot know a TagStore fronts it, but tag routes of its networks are answered locally, so this deployment truthfully supports tags for them. Entries without a declaration (full core networks) pass through untouched.
