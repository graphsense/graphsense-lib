...
#### Added
- **Fast PySpark one-off UTXO clustering + incremental delta clustering.** `transformation cluster` runs a PySpark path (`run_clustering_spark`): it bulk-reads `raw.transaction` and `address_ids_by_address_prefix` via parallel token-range scans, streams each Spark partition to the in-process Rust Union-Find as zero-copy Arrow buffers (`Clustering.process_transactions_arrow`, no Python materialization), and bulk-writes `fresh_address_cluster` / `fresh_cluster_addresses` plus a `fresh_cluster_stats` (size + min_address_id per cluster) aggregate via the Spark Cassandra connector. The delta updater's incremental clustering is a targeted cluster-delta: it reads only `address→cluster` for the touched addresses and their `fresh_cluster_stats`, classifies each connected component as new/join/merge, and reads full membership (`fresh_cluster_addresses`) only for the smaller absorbed side of a merge — issuing all reads one partition at a time with `fetch_size` paging (`CassandraDb.read_partitions_concurrent`) so a large multi-partition `IN` can no longer ReadTimeout on big exchange clusters. Adds the `fresh_cluster_stats` table (`transformed_utxo` schema migration v1→v2).
- **Fresh clusters are served through self-describing entity ids — no REST switch, no restart.** Fresh cluster ids are published shifted by `FRESH_CLUSTER_ID_OFFSET` (2^32): an entity id at or above the offset routes to the fresh tables (keyed by the raw id below the offset), anything below to the legacy `cluster` table, so both id spaces coexist on the same keyspace. The shift is a REST-boundary translation only — Cassandra and the tagstore keep raw ids (`cluster_id == min(address_id)`); tag lookups unshift via `tagstore_cluster_id`. Address responses carry a `fresh_cluster_id` field (shifted) once fresh clustering is active on the keyspace. `in_degree`/`out_degree` were dropped from `fresh_cluster_stats` (no relations shuffle in the recompute); REST fills them from the root address's legacy cluster row, and cluster relations (neighbors, links, cluster txs) are served via the same legacy hop.
- **A per-keyspace bootstrap marker replaces the `GRAPHSENSE_FRESH_CLUSTERING_ENABLED` env var.** `transformation cluster` writes a `fresh_clustering_active` row into the transformed keyspace's `state` table as its final step; the delta updater maintains the fresh tables and REST fills `fresh_cluster_id` iff the marker is present (mere existence of the fresh tables is not sufficient — migrations create them empty on every keyspace, and clustering from a mid-chain starting point would build garbage). Enabling fresh clustering for a currency is therefore just: run the one-off bootstrap.
- **`GRAPHSENSE_TAGSTORE_FRESH_CLUSTERS` flips the tagpack cluster-mapping feeder (read source, write target, MV refresh) to the `*_v2` relations**, in lockstep with the existing read switch. The feeder reads cluster membership from `fresh_address_cluster` / `fresh_cluster_stats`, writes `address_cluster_mapping_v2`, and refreshes `tag_count_by_cluster_v2` / `best_cluster_tag_v2`. Exclusive switch; the first v2 fill needs a full `--rerun-cluster-mapping-with-env`.
...
- **REST cluster reads synthesize fresh-clustering singletons instead of 500ing.** Because fresh clustering stores only multi-member clusters, a singleton id has no `fresh_cluster_stats` row: `get_entity` now synthesizes the one-address entity, `list_entities` restores singletons dropped from the bulk lookup (preserving input order and omitting only genuinely unknown ids, matching legacy behavior), and stats-pending rows created by the delta updater are healed from their membership. UTXO only, in the fresh id space (`id >= FRESH_CLUSTER_ID_OFFSET`); legacy ids keep exact legacy semantics.
