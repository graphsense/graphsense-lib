...
#### Added
- **`transformation delta-to-raw` can bulk-write through the Cassandra Sidecar (`--writer sidecar`, UTXO chains).** The `UtxoTransformation` write path routes every table through `bulk_write_dataframe` instead of the CQL connector, with the Spark session assembling the cassandra-analytics package and SSTable-writer JVM flags at launch (requires `spark.local.dir` in the Spark profile). Defaults from `full_transform_args.sidecar` (same block as `raw-to-transformed`): `contact_points`, `local_dc`, `consistency_level`, and `enabled` as the no-flag default on UTXO chains. Misconfiguration (missing contact points, `LOCAL_*` without `local_dc`) fails before Spark starts, not at the first write.
- **`transformation delta-to-raw --exchange-rates-from <keyspace>` seeds the target keyspace's `exchange_rates` from an existing raw keyspace.** The Delta lake carries no exchange rates, so a rebuild into a fresh keyspace previously ended without them (the API-based `exchange-rates … ingest` re-fetch being the only fill). The copy runs driver-side before the Spark job and fails loudly when the source table is empty.
...
#### Fixed
- **UTXO Delta-lake ingest stores `txinwitness` as raw bytes, and `transformation delta-to-raw` heals existing hex-form rows in flight.** The node's JSON-RPC delivers witness elements as hex strings; the Cassandra ingest path unhexes them (`tx_io_summary`), but the parquet prep converted only `spent_transaction_hash` — so every Delta lake row stored the UTF-8 bytes of the hex text in the binary witness column, and a rebuild copied that into the raw keyspace. Structural consumers can't parse the hex form: the cross-chain pubkey extractor never found a witness-borne pubkey (P2WPKH/P2WSH spenders — most modern activity — were invisible to pubkey linking). The prep now unhexes witness on the Delta path, and the transform decodes hex-form lists (list-level all-or-nothing: only when every element is valid even-length lowercase hex-ASCII, so genuinely raw rows from a future clean lake pass through untouched). The encoding is lossless, so already-written lakes remain fully healable at rebuild time.
- **The delta-to-raw normalization UDF works on minimal Spark-executor environments: `normalize_base58_p2pkh` moved to the lean `utils/utxo_address` module.** The UDF pickles the function by module reference, so executors import its defining module — which was `ingest/utxo.py`, whose module top pulls in methodtools, the Cassandra driver and the RPC exporters, none of which exist in the baked spark-env archive (`ModuleNotFoundError: methodtools` on every task). The normalizer and `_NETWORK_SCRIPT_PARAMS` now live in `graphsenselib/utils/utxo_address.py` (stdlib-only at import time) and are re-exported from `ingest.utxo` for the ingest-side callers; the image build's spark-env smoke test now exercises the normalization to keep it importable there.
- **`transformation delta-to-raw` now normalizes BTC-form base58 P2PKH addresses to the network's version byte.** The Delta lake stores derived address strings, so rows written by pre-2.14.4 ingests still carry BTC-form (`1…`) fallback addresses (e.g. LTC P2PK outputs and their spends) — and a rebuild copied them verbatim into the new raw keyspace, re-importing the corruption the ingest-side fix (below) stopped at the source. `transform_transaction` now runs `normalize_base58_p2pkh` over `inputs[].addresses`/`outputs[].addresses`; networks where the rewrite is a no-op (BTC's own version byte, unknown networks) skip the pass entirely.
- **UTXO ingest normalizes BTC-form base58 P2PKH addresses to the network's version byte.** Pre-2.14.4 ingests derived fallback addresses with BTC's version byte on every network, so LTC P2PK outputs (and their spends, via input resolution against those stored rows) carry `1…` forms instead of `L…`. `enrich_txs` now re-encodes such addresses on every ingest path; `normalize_base58_p2pkh` is importable standalone for healing jobs over already-stored data.
