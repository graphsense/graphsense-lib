...
- **`build_pathfinder_file` node layout accounts for multi-line labels.** The hierarchical and columnar layouts (`convert/gs_files/encoder.py`) now size each node's vertical slot from its label — a label wrapping past ~12 characters per line widens the gap to its column neighbours so the text does not overlap. Unlabelled / single-line layouts are unchanged.
