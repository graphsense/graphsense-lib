...
#### Added
- **Arrow-based feed + bounded shuffle width for the Spark clustering path.** `run_clustering_spark` now pulls the multi-input edge sets to the driver via Arrow IPC (`DataFrame._collect_as_arrow`, columnar/C-speed) instead of `toLocalIterator`, which transferred rows one-by-one over py4j (~50k rows/s) and launched one Spark job per partition (hundreds of stages). The `list<int32>` column is converted with `to_pylist()` and fed to the Rust Union-Find in `feed_batch_size` slices; falls back to `toLocalIterator` if the private Arrow collector is unavailable. A new `read_partitions` parameter (default 64, CLI `--read-partitions`) caps `spark.sql.shuffle.partitions` and coalesces the edge-set DataFrame so the distinct/join/groupBy don't fan out into tiny tasks. Note: `_collect_as_arrow` materializes the full edge set on the driver at once (not streaming) — compact in Arrow (~tens of bytes/tx) but O(result); the slice feeding bounds only the larger Python-list copy.
