...
- **`build_pathfinder_file` node layout accounts for multi-line labels.** When a node label wraps past ~12 characters per line, the hierarchical and columnar layouts (`convert/gs_files/encoder.py`) widen that column's uniform row step to fit the tallest label, keeping the column evenly aligned. Unlabelled / single-line columns are unchanged.
