...
- **`build_pathfinder_file` backend-aware verifier (new `verify` flag, default `true`).** A new module `src/graphsenselib/mcp/tools/pathfinder_verify.py` exposes a minimal async `GraphsenseBackend` Protocol (two methods: `address_exists`, `tx_addresses`) and a `verify_spec` that runs three backend-aware checks: every address is looked up, every tx hash is looked up, and for each `agg_edges.tx_ids` reference both edge endpoints are asserted to be in the tx's on-chain participant set. The shipped `RestBackend` adapter uses public REST paths (`/{network}/addresses/{address}`, `/{network}/txs/{tx_id}`) so any deployment works in-process or out-of-process; concurrency is capped via a semaphore (default 8) to avoid the pool-exhaustion shape from 2026-05-04. Findings merge into `summary.warnings` AND the TextContent block. Backend transport errors are downgraded to a single "verifier unavailable" warning so a flaky backend cannot sink a structurally valid file. Pass `verify=false` to skip during fast iteration.
- **`build_pathfinder_file` structural stray-address warning.** `_collect_warnings` now flags addresses listed in `addresses` that are not referenced from any `agg_edge` endpoint AND not declared `starting_point=true` — they would render as floating address nodes with no relationship to anything else. `starting_point` remains the explicit opt-out for legitimate anchor-only graphs.

#### Changed
- **`build_pathfinder_file`: tidy-tree hierarchical layout, UI-matched spacing, and a `download_url` channel.** The hierarchical layout (used whenever a node is flagged `starting_point=true`) now builds a BFS spanning tree and walks it post-order, so every descendant stays on the same side as its ancestor and branches no longer cross; txs snap to the mean y of their endpoints. Layout constants (`_HIER_Y_STEP`, `GsBuilder._ROW`) moved from `3.0` to `2.5` to match the Pathfinder UI's own defaults (`Config/Pathfinder.elm`). When a file store is configured the tool stashes the `.gs` file and returns an additive `download_url` in `structured_content` (the channel weak MCP hosts can still use); the embedded resource is still sent unless `file_store.embed_resource=false`. The tool is now async. Backwards compatible — without `file_store` config the only change is `download_url: null`.
- **`build_pathfinder_file` label guidance.** Docstring + `label` field descriptions tell the model not to restate attribution tags or transaction date/value (the UI already shows them) and to reserve `label` for case context. Encoding unchanged.

#### Fixed
- **`build_pathfinder_file` docstring no longer claims the model receives the `.gs` bytes.** The bytes travel in a resource channel the model cannot read; the old wording made hosts that drop the embedded resource fabricate base64 / `data:` URLs.
...
- **`build_pathfinder_file` robustness on weak MCP hosts.** Two Le-Chat-driven fixes: (1) `PathfinderSpec` now declares an optional `layout` field so the very common LLM mistake of nesting `layout` inside `spec` no longer fails with a Pydantic `extra_forbidden` validation error — when the top-level argument is the default (`"auto"`) and `spec.layout` is set, the latter wins; explicit top-level `layout` still takes precedence. (2) The tool always appends a `TextContent` block carrying the download URL (or an "embedded in this response" line) so hosts that only render `content` and ignore `structured_content` still surface a usable response to the user.
- **`build_pathfinder_file` no longer stacks multi-tx edges on a single point.** The 2.13.5 tx-snap-to-midpoint behaviour assigned every tx on a one-anchor↔one-destination edge the SAME `(x, midpoint_y)`, so an N-tx edge rendered as a single visually visible tx (observed: 73 of 83 txs invisible). `apply_hierarchical_layout` now does a per-column single-linkage clustering pass (threshold `y_step / 2` — sibling edges at exactly `±y_step/2` stay on their straight lines as before) and spreads each pile symmetrically along y with the same row pitch addresses use. The pass iterates until stable so a pile's wider spread doesn't push spread slots onto a previously-singleton neighbour.
- **`build_pathfinder_file` tx-identifier guidance.** `_TxSpec.id` and the docstrings of `lookup_tx_details`, `list_txs_for`, and `list_tx_flows` now spell out that on account-model chains the response carries both `tx_hash` and `identifier`, and that pathfinder needs the `identifier` (which uniquely names a sub-payment within a tx hash — native transfer vs token transfers) rather than the bare `tx_hash`; on UTXO chains the response has only `tx_hash`. Description-only change.
- **`build_pathfinder_file` warnings now surface in the TextContent block, not only `structured_content`.** Hosts that drop `structured_content` (observed for Mistral Le Chat) would otherwise ship a `.gs` with hidden warnings — the original 9e44196... incident, where a tx listed in `txs` but missing from any `agg_edges.tx_ids` produced a broken graph the agent never noticed. The TextContent now appends a `Warnings — fix the spec and rebuild:` section listing every entry from `summary.warnings`, so even content-only renderers expose the warnings to the agent and user.
- **Verifier `RestBackend.tx_addresses` now requests `include_io=true` & `include_nonstandard_io=true`.** UTXO `inputs` / `outputs` are declared `Optional[...] = None` on the REST response model and are EXCLUDED from the body when not requested. Without the params, the adapter returned an empty address set for every UTXO tx — surfaced as the misleading warning `tx involves {}` for every tx-mediated edge in user-reported builds, even on byte-identical files that had previously passed verification. Account-model bodies carry `from_address` / `to_address` unconditionally, so the flags are no-ops there. Both params are now sent on every tx lookup; locked in by a regression test that asserts the query string.
- **`_ID_PATTERN` accepts underscore so account-model sub-payment identifiers validate.** `build_pathfinder_file`, `lookup_tx_details`, `list_txs_for`, `list_tx_flows` and the other consolidated MCP tools previously rejected the documented identifier form `<hash>_I<n>` (internal trace) / `<hash>_T<n>` (token transfer) with `Invalid tx hash` — agents misread that as a verify finding and re-tried with `verify=false`, which didn't help (the rejection happened at input boundary, before the verifier). The pattern is now `^[a-zA-Z0-9_]{1,150}$`; the error message is reworded to "<name> X has an invalid format (allowed: …). This is a format check at the input boundary, NOT a verify finding." The `_TxSpec.id` docstring is rewritten to make the bare-`tx_hash` vs sub-payment-identifier choice explicit instead of nudging agents toward the suffixed form unconditionally.
