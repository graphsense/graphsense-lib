...
- **`transformation pubkey-update` / `pubkey-compact` — cross-chain pubkey → address lookup.** ⚠️ **ALPHA** — not yet validated in production; the command interface and behaviour may change, and invoking either prints an alpha warning. `pubkey-update` reads new transactions from a currency's source Delta Lake, extracts signing pubkeys, merges them into a shared cross-chain Delta store (`observed`/`materialised`/`state` under `--sink-path`), and writes derived addresses for any pubkey newly observed on 2+ chains to the configured `--sink-type` backend (Cassandra or a Delta table). `pubkey-compact` deduplicates/bin-packs the append-only `observed` table between runs (detection-neutral). For multi-chain backfills, `pubkey-update --skip-detect` appends a chain's pubkeys to `observed` (and bumps state) without running the cross-chain detection/materialisation step, and a new currency-agnostic **`transformation pubkey-detect`** command runs that detection once over the fully-appended store — so the full-table detection groupBy executes a single time instead of once per chain (`backfill.sh` does this by default, via `SKIP_DETECT`). The Cassandra write keyspace defaults to a fresh `pubkey_v2` (not the legacy `pubkey` table an older script may own) and is overridable per-run via `--pubkey-keyspace` or per-env via `environments.<env>.pubkey.keyspace`; the REST reader selects its source independently via `cross_chain_pubkey_mapping_keyspace` (still defaulting to legacy `pubkey`). Helper scripts under `scripts/pubkey/` (`backfill.sh`, `diff.py`) drive a full-history backfill and diff the result against the legacy table. **`transformation pubkey-load`** separately loads the `pubkey_by_address` Delta table produced by a `--sink-type delta` run into Cassandra, so the heavy extraction + cross-chain detection can run delta-only (no production-Cassandra stress) and be reviewed before the throttled Cassandra write. Extraction coverage: UTXO input side (P2PKH, P2WPKH, nested P2SH-P2WPKH, P2SH-P2PKH, and P2SH/P2WSH multisig) plus output side (P2PK and bare P2MS); the account side recovers the ETH/TRX signing key and, for ETH, drops rows whose recovered address ≠ the stored `from_address` as a correctness self-check (TRX skips it — the owner can legitimately differ from the signer for multisig/permission accounts). Address derivation adds Bitcoin Cash CashAddr. BCH defaults its start block to the fork height (`478558`) when none is given, so the shared pre-fork BTC history isn't re-extracted into trivial cross-chain collisions (overridable via `--start-block`). secp256k1 public-key validation (`is_valid_secp256k1_pubkey`) now uses coincurve/libsecp256k1 by default (`fast=True`), ~1000× faster than the pure-Python reference path (the cofactor-1-redundant subgroup check is dropped). For Spark standalone clusters whose executors lack graphsenselib + native deps, the Docker image bakes a minimal (~13 MB) site-packages archive at `/opt/graphsense/spark-env.tar.gz`, shipped to executors via `spark.archives` + `spark.executorEnv.PYTHONPATH` (executors use their own Python 3.13; no interpreter is shipped). Additional helper scripts: `scripts/pubkey/validate_account_fieldmap.py` (validate the account field mapping on real delta rows), `scripts/pubkey/profile_legacy_driver.py` (read-only profile of the legacy keyspace), and `scripts/pack_spark_env.sh` (build a relocatable env for any Python-UDF Spark job).
