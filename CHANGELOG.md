...
- **The new serving-path query optimizations are now switchable via one config flag, and gaps in `token_configuration` are surfaced as warnings.** A new database config option `fanout_bounding_and_links_precheck_enabled` (default `true`, env `GS_CASSANDRA_ASYNC_FANOUT_BOUNDING_AND_LINKS_PRECHECK_ENABLED`) controls both optimizations that trust precomputed aggregate data: the per-token fan-out bounding in address/entity tx listings and links (derived from the address rows' `total_tokens_received`/`total_tokens_spent` maps) and the links relations pre-check (immediate empty result when no directed edge exists, early stop once all `no_transactions` edge txs are found). Setting it to `false` restores the previous unbounded/full-scan behavior — an escape hatch in case the aggregates are incomplete (e.g. written by an outdated transform) and token txs go missing from listings or links miss txs / come back empty. Independently of that flag, the serving path now logs a warning whenever an address's token maps reference a token that is **not** in `token_configuration`: such txs are invisible to the per-token queries with *or* without bounding (only configured tokens are ever queried), so the warning points operators at the actual fix — adding the token to `token_configuration` — rather than at the bounding.
...
- **UTXO ingest warns when spent inputs remain unresolved.** After the cache + `getrawtransaction` resolution phases, any input still lacking a value was written with null value/addresses and its transaction's `input_value`/`fee` (summed over resolved inputs only) silently understated. The count of unresolved inputs is now logged as a warning per batch.
- **Token undecodable-transfer drops are logged.** The ERC-20/TRC-20 `Transfer` decoder swallowed every exception and returned `None` with no signal (an in-code TODO admitted it); it now logs a warning with the log index and token address before dropping.
