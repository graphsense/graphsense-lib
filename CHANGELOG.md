...
- **`GRAPHSENSE_FRESH_CLUSTERING_ENABLED` now also flips the REST cluster reads to the fresh tables.** The address-level `fresh_cluster_id` lookup is fixed to include the `address_id_group` partition key (the previous `WHERE address_id=?` was unservable on a partitioned table), and entity reads (`get_entity`, `list_entities`) now read `fresh_cluster_stats` while cluster-membership (`list_entity_addresses`) reads `fresh_cluster_addresses` when the flag is on (UTXO only; column-compatible with the legacy `cluster`/`cluster_addresses` tables, so response shaping is unchanged). Legacy is the default.
