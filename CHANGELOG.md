...
#### Changed
- **The token fan-out bounding is now configurable, and gaps in `token_configuration` are surfaced as warnings.** A new database config option `token_fanout_bounding_enabled` (default `true`, env `GS_CASSANDRA_ASYNC_TOKEN_FANOUT_BOUNDING_ENABLED`) restores the old unbounded per-token fan-out in address/entity tx listings and neighbor links when set to `false` — an escape hatch in case an address row's `total_tokens_received`/`total_tokens_spent` aggregate maps are incomplete (e.g. written by an outdated transform) and token txs go missing from listings. Independently of that flag, the serving path now logs a warning whenever those maps reference a token that is **not** in `token_configuration`: such txs are invisible to the per-token queries with *or* without bounding (only configured tokens are ever queried), so the warning points operators at the actual fix — adding the token to `token_configuration` — rather than at the bounding.
