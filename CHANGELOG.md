...
#### Added
- **Zero-copy Arrow feed from the Spark read loop into the Rust Union-Find.** `run_clustering_spark`'s per-partition feed previously did `Arrow.to_pylist()` → `process_transactions(list[list[int]])`, materializing tens of millions of Python list/int objects on the driver per partition — single-threaded, GIL-bound, and on a real BTC-scale run the single largest cost (`Arrow→pylist` 259 s of a 461 s read phase, vs 14 s for the actual union-find). A new Rust method `Clustering.process_transactions_arrow(pyarrow.ListArray)` (in the in-repo `gs_clustering` crate) imports the Arrow `list<uint32>` via the C Data Interface and reads the offsets+values buffers in place — zero Python objects; the executor-side IPC now emits `uint32` to match. The driver feed loop calls it directly, collapsing the `pylist` cost to ~0 (verified equivalent to the old Vec path, including the IPC round-trip and offset-correct sliced feeds; wrong inner types are rejected). The instrumentation drops the `pylist` timer accordingly. The legacy `process_transactions(Vec<Vec<u32>>)` is retained for the single-driver CQL path and the delta updater.
- **Cassandra Sidecar SSTable bulk-write path for Spark clustering (`--writer sidecar`).** `run_clustering_spark`'s write phase can now stream SSTables into Cassandra through the Sidecar via the cassandra-analytics bulk writer (`org.apache.cassandra.spark.sparksql.CassandraDataSink`), bypassing the spark-cassandra-connector's CQL coordinator/commitlog/memtable path — a port of graphsense-ethereum-transformation's `SidecarBulkWriter` to PySpark. A new `transformation/sidecar.py` holds `bulk_write()` (the data-source call: `sidecar_contact_points` / `local_dc` / `bulk_writer_cl` / `number_splits=-1` / `data_transport=DIRECT`) and `sidecar_spark_config()`, which reproduces cassandra-analytics' programmatic `BulkSparkConf.setupSparkConf` + `KryoRegister.setup` as SparkSession-builder configs (KryoSerializer, both Kryo registrators including the version-specific `KryoRegister$Vxx`, the JDK11 `--add-opens` block, and `-XX:hashCode=0`) — PySpark cannot run them before the JVM/classpath exist. `create_spark_session` adds the `cassandra-analytics-core_spark3_2.12:0.3.0` package and those configs when `writer=sidecar`. New CLI options on `transformation cluster`: `--writer cassandra|sidecar` (default cassandra), `--sidecar-contact-points`, `--sidecar-local-dc`, `--sidecar-consistency-level` (default LOCAL_QUORUM), and `--sidecar-cassandra-version` (default 4.1.11; selects the Kryo registrator and the SSTable format the writer generates). The default connector write path is unchanged.
- **Spark clustering write phase: int64 Arrow fast path, singleton skip, py4j log mute.** Three fixes to `run_clustering_spark`'s bulk write, after the per-slice timing logs pinned `build+count` at ~94 s/slice (vs ~9 s for the actual Cassandra writes) on BTC. (1) **int64 cast before `createDataFrame`.** Rust's mapping is `uint32`; Spark Arrow has no unsigned types, so `createDataFrame` silently fell back to a row-at-a-time driver path — ~19× slower warm (measured: 5M rows uint32 56 s vs int64 2.8 s) AND it pickles every row into oversized task buffers, which on large chains OOMs the driver JVM scheduler (`SerializableBuffer`/`launchTasks` heap death). Feeding int64 numpy engages the Arrow path (500 partitions); Spark casts down to the tables' int32. int32 also misses the Arrow path, so int64 is required. (2) **Skip singletons by default** (`skip_singletons`, CLI `--keep-singletons` to restore): write only addresses in a multi-address cluster — an address absent from `fresh_address_cluster` is its own cluster — dropping the majority of rows on most chains. A singleton is a cluster of SIZE 1, not merely `cluster_id == address_id`, so the filter keeps cluster roots (verified against a brute-force size≥2 check). (3) `create_spark_session` mutes the `py4j` logger to WARNING so its per-call `clientserver` DEBUG spam (a PySpark-wide concern) stops drowning the clustering DEBUG lines at `-vvv`.
- **Spark clustering observability, `--materialize` A/B toggle, and partition prefetch.** `run_clustering_spark` now emits an extensive per-phase / per-partition / per-write-slice timing breakdown at INFO so a real cluster run states exactly where the wall-clock goes instead of it being inferred. The read loop times each partition's `wait(spark)` (blocked in `toLocalIterator`: Spark compute + executor→driver Arrow ship) separately from the driver `feed` (Arrow→pylist + Rust union-find), then prints an end-of-phase verdict naming the dominant cost; phase 2 times `get_mapping()`, phase 3 times each Cassandra write (`fresh_address_cluster` / `fresh_cluster_addresses`) per slice, and a final line reports the read/map/write `TOTAL`. `toLocalIterator(prefetchPartitions=True)` now overlaps the next partition's compute+ship with the current feed, reclaiming the otherwise-serial driver pull. A new `--materialize` flag (`run_clustering_spark(materialize=...)`, default off) `persist()`+`count()`s the edge sets so the scan+join+groupBy computes once with full cluster parallelism before streaming, instead of `toLocalIterator` driving the final reduce one partition at a time — an A/B knob whose win is cluster/data dependent (it did not help on synthetic local data, where recompute is free), with the timing logs reporting which way is faster on real data. Motivated by profiling the driver feed in isolation at ~590k txs/s (Rust union-find ~7.7M txs/s; Arrow→pylist ~0.6M txs/s) — ~11× faster than the observed ~52k txs/s end-to-end, i.e. the bottleneck is the Spark read/join/ship, not the feed. No change to clustering output or to the default (non-`--spark`) path.
- **`transformation cluster` flag ergonomics.** `--write-chunk` now applies to whichever path runs (previously silently ignored under `--spark`): it defaults to None and each path falls back to its own write-slice default (100k CQL, 5M Spark) when unset, or honors the explicit value otherwise. `--feed-batch-size`'s help was rewritten to make clear it is a per-partition memory bound (caps the Arrow→Python-list conversion per Rust call) that only binds when a partition exceeds it — large chains or low `--read-partitions` — rather than a throughput knob with no visible effect at small scale.
- **Spark clustering perf fixes: decouple join parallelism from streaming width, Arrow-fast write.** Two fixes to `run_clustering_spark`: (1) it no longer pins `spark.sql.shuffle.partitions` to `read_partitions` — that capped the address-resolution join (against the full hundreds-of-millions-row `address_ids_by_address_prefix` table) at a handful of huge, spilling partitions. `read_partitions` now only coalesces the final edge-set DataFrame for bounded streaming; join parallelism uses the session default (200 + AQE) and is tuned via `spark.sql.shuffle.partitions`. (2) The bulk-write now enables `spark.sql.execution.arrow.pyspark.enabled` before `spark.createDataFrame(pandas)`; measured locally, that cuts the per-chunk conversion of 5M rows from ~41 s to ~0.15 s (~270×) and yields 500 vs 4 output partitions (so the Cassandra write also parallelizes far better). Also hardened the executor-side partition function to import pyarrow locally rather than via closure capture.
- **Bounded Arrow-IPC partition streaming + bounded shuffle width for the Spark clustering path.** `run_clustering_spark` streams the multi-input edge sets to the driver **one Spark partition at a time**: each executor serializes its partition to an Arrow IPC blob with pyarrow and `rdd.toLocalIterator()` pulls one blob per partition, which is then fed to the Rust Union-Find in `feed_batch_size` slices. This replaces `DataFrame.toLocalIterator` (py4j row-by-row, ~50k rows/s, one job per partition → hundreds of stages) while staying bounded — each transfer is ~`total/read_partitions`, so it never trips `spark.driver.maxResultSize` (a single whole-frame `_collect_as_arrow` does: the LTC edge set alone is >1 GiB). Uses only public APIs (no private `_collect_as_arrow`, no `spark_partition_id` filtering) and is O(partitions), not O(partitions²). A new `read_partitions` parameter (default 64, CLI `--read-partitions`) caps `spark.sql.shuffle.partitions` and coalesces the edge-set DataFrame so distinct/join/groupBy don't fan out into tiny tasks, and sets the per-blob size (raise it if a single partition exceeds `maxResultSize`).
- **PySpark bulk read/write path for one-off UTXO clustering (`transformation cluster --spark`).** The single-driver clustering path (`run_clustering_one_off_from_cassandra`) reads `raw.transaction` block-by-block via serial point/range CQL queries through the coordinator and writes the mapping back via async prepared `INSERT`s — read-bound and slow for full-chain clustering. A new `run_clustering_spark` (in `transformation/clustering.py`) bulk-reads `raw.transaction` and `address_ids_by_address_prefix` via the Spark Cassandra connector (parallel token-range scans across the cluster), derives each multi-input transaction's distinct input `address_id` set (dropping coinbase, mirroring the single-driver semantics), feeds those order-independent edge sets to the in-process Rust Union-Find (`gs_clustering`) on the driver via `toLocalIterator` (driver memory bounded by `feed_batch_size`), then bulk-writes the resulting `address_id → cluster_id` mapping to `fresh_address_cluster` / `fresh_cluster_addresses` via the connector in `write_chunk` slices (casting to the tables' int32 columns, dropping the coinbase placeholder `address_id 0`). Exposed via new `--spark` / `--local` flags on `transformation cluster`; reuses the existing `create_spark_session` and `spark_config` / `spark_packages` config. The Spark path clusters the whole transaction table (block sub-ranges are not applied); `max_address_id` still comes from `summary_statistics.no_addresses`. The default single-driver path is unchanged.
- **Environment variable substitution in config files (`${VAR}`).** Any string value in a config file may now reference environment variables, resolved at load time against `os.environ`. A new recursive helper `resolve_env_vars` (in `utils/generic.py`) walks dicts/lists and substitutes placeholders inside string values; non-string scalars pass through. Syntax: `${VAR}` (raises a clear error if unset and no default given), `${VAR:-default}` (default when unset/empty, use `${VAR:-}` to allow empty), and `$${VAR}` to emit a literal `${VAR}` without substitution (a bare `$VAR` without braces is never touched). Wired into every file→dict→typed config path: the CLI config's `AppConfig.load_partial`, the goodconf-native `AppConfig.load()` path (via a `_EnvResolvingFileConfigSource` settings source so the `get_environment` fallback is covered too), and the REST config's `web/app.py:load_config` (covering all `GSRestConfig.from_dict` branches, including the `.graphsense.yaml` `web` key). Useful for keeping secrets (DB URLs, credentials) out of committed config files. Backwards compatible: configs without `${...}` placeholders are unchanged.
- **Overridable Spark Maven packages via `spark_packages` config.** The Spark transformation session's Maven packages were hardcoded in `transformation/spark.py`. They are now defined in a `DEFAULT_SPARK_PACKAGES` dict keyed by logical name (`cassandra_connector`, `joda_time`, `delta_spark`, `hadoop_aws`) and can be overridden per-package via a new top-level `spark_packages` config field (`Dict[str, str]`), accessed through `AppConfig.get_spark_packages()` and threaded through `transformation/cli.py` → `factory.run` → `create_spark_session`. Overrides are merged over the defaults (`{**DEFAULT_SPARK_PACKAGES, **spark_packages}`), so you only list what you want to change, e.g. `spark_packages: {hadoop_aws: "org.apache.hadoop:hadoop-aws:3.3.4"}`; defaults are unchanged when the field is empty. `hadoop_aws` is still only added when s3 credentials are present, and the full-replace `spark_config: {spark.jars.packages: "..."}` escape hatch (applied last) continues to win.
- **Baseline inheritance for `s3_configs` entries.** A reserved `baseline` key under `s3_configs` now holds shared fields (typically `AWS_ENDPOINT_URL` / `AWS_REGION`); every other named entry inherits from it and its own keys win on conflict. Resolution happens in `AppConfig.get_s3_credentials` as `{**baseline, **named}`, so all existing call sites (ingest CLI, transformation CLI, delta-updater) pick it up transparently. Asking for `baseline` by name returns it as-is (no self-merge), and configs without a `baseline` key behave exactly as before. Eliminates per-user duplication when the only thing differing between credential sets is the access key / secret pair.
- **Baseline inheritance + named profiles for `spark_config`.** `spark_config` now accepts either the legacy flat shape (Spark property names as top-level keys) or a nested shape with a reserved `baseline` profile and any number of named profiles that override it. A new `AppConfig.get_spark_config(profile_name=None)` accessor auto-detects the shape: flat configs are returned as-is (passing a profile name in flat mode raises a clear "convert to nested form" error so misconfigs are obvious); nested configs return `baseline` alone when no name is given, or `{**baseline, **profile}` for a named profile (`baseline` itself returns unmerged). The single existing consumer in `transformation/cli.py` was switched to `get_spark_config()` so the nested form works end-to-end without selecting a profile. Field type relaxed from `Dict[str, str]` to `Dict[str, Any]` to accept both shapes; fully backwards compatible with existing flat configs.
