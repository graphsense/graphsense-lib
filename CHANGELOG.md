...
- **CI no longer skips `slow` tests, and the Spark job no longer path-filters.**
  ...
  every push and PR.
- **Dependency bumps: the `uv-minor-patch` group (22 packages), setuptools
  84.0.0, the `docker-images` group (uv 0.12.9, a new temurin 11 digest) and
  arrow 59.3.0 in `rust/gs_clustering`.** Notable inside the group: fastapi
  0.136.3 -> 0.141.1, sqlmodel 0.0.38 -> 0.0.42, rapidyaml 0.13.0 -> 0.15.2,
  redis 8.0.0 -> 8.1.0, cassandra-driver 3.30.0 -> 3.30.1, ty 0.0.73 -> 0.0.79.
  **`ruff` is deliberately held at 0.15.17**: 0.16 widens the default rule set
  and reports 3442 findings (2741 autofixable) on a tree that is clean today,
...
- **Every `ArrowInvalid` from the delta sink took the shrink-the-row-group
  ...
  operand is a bare string literal, so the condition is always true and the
  `else: raise e` beneath it was dead code. An unrelated Arrow error therefore
  spent up to 20 attempts halving `max_row_group_size` before surfacing as
  "Could not write delta-file after 20 attempts", which names neither the real
  error nor the file. Found by the `ty` bump below, which is the entire reason
  that bump is worth taking.
...
#### Fixed
- **Exchange-rate fetches can no longer hang an ingest forever.** Every HTTP call in `rates/` was issued without a timeout — the four provider requests (`coingecko`, `coinmarketcap`, `cryptocompare`, `coindesk`, plus the token-rates contract lookup) and the ECB FX download, which `pd.read_csv(FX_URL)` performed through `urllib.request.urlopen` with the socket default of `None`. With no read timeout and no `SO_KEEPALIVE`, a connection that is established and then silently dropped (a NAT/firewall/LB reaping an idle flow) parks in `recv()` with no OS-level timer that will ever wake it. This was worse than a stalled run: `ingest` performs the exchange-rates step *before* it acquires the ingest lock, so a wedged process held nothing, blocked nothing, and was invisible to the lock's own stale-holder alerting — the next cron tick simply started another ingest that ran fine while the corpses accumulated, unnoticed until someone looked at `ps`. All six call sites now pass a shared `HTTP_TIMEOUT` of `(5, 60)` `(connect, read)`, and the ECB zip is fetched with `requests` into a buffer (`rates.utils.read_csv_url`) rather than by pandas. The retrying adapter the providers already mounted is now shared as `rates.utils.rates_session()`; because it retries read timeouts on GET, a transient stall is still retried and only a persistent one fails, in bounded time. Note the read timeout bounds *silence between reads*, not total request duration — which is exactly the failure being fixed, not a new cap on legitimately slow responses.
- **Two remaining reader paths no longer misread wrapped 32-bit tx counts.** `get_addresses_light` (the delta updater's change-detection read) fed raw counts into its "did anything change" comparison, and the top-untagged job summed raw `no_incoming_txs + no_outgoing_txs` for its ranking and `min_txs` filter — a wrapped negative (TRON USDT: −591M for ~3.7e9 real txs) made the busiest address look nearly idle in both. Both now saturate first (`saturate_int32_count` on the read path, a `when(col < 0, Int.MaxValue)` column expression Spark-side), matching the REST read path's existing normalization; true bigint values from rebuilt keyspaces pass through unchanged.
- **Dependabot stopped proposing dependency updates it had already been asked for.** Version updates for the `uv` ecosystem hit the default `open-pull-requests-limit` of 5 and stalled, so nothing was bumped while advisories accumulated; and three manifests were not configured at all — `clients/python` and `tests/regressions` (each its own uv project with its own lockfile) and `rust/gs_clustering` (cargo), which is why the `pyo3` alerts never produced a pull request. `.github/dependabot.yml` now covers all three, groups uv minor+patch bumps into one weekly PR per project so the PR slots stay free, and groups `pyo3` with `arrow` for cargo, since the two only resolve when they move together. `spark/` gained coverage too: `directory: "/"` picks up the two root Dockerfiles but not subdirectories, so `spark/Dockerfile` (`eclipse-temurin:11-jdk`) had none — it now has its own `docker` entry, carrying the same Java-11 major/minor `ignore` as the runtime image's `java11` stage. Note that closing a Dependabot PR unmerged tells Dependabot not to re-propose that release — a manual bump of the same dependency is the way to clear the alert.
...
#### Changed
- **/links slow tail fixed: pathological requests drop from a median 26.5s to 2.3s (worst cases from 100s+ gateway kills to seconds), the overall median from 6.9s to 1.3s, with ~48x fewer Cassandra queries and results and page tokens byte-identical; no band regresses.** The endpoint intersects two addresses' tx histories with no edge→tx index; the async read path now (a) grows the candidate batch adaptively up to `links_adaptive_fetch_size_cap`, (b) scans consecutive empty block buckets concurrently (`txs_secondary_group_scan_window`), (c) probes the larger side only in the direction and asset a link can actually have, (d) fetches large probe rounds as single-partition `transaction_id IN (...)` batches (`links_probe_in_batch_size`, activating above `links_probe_in_min_statements`), (e) races the opposite scan direction for sparse edges after a 1s hedge, but only when the scanned side's tx count exceeds a size bound (`links_sparse_direction_race_enabled`, `links_direction_race_min_candidates` — fixes ascending scans whose links sit at the end of a huge history without spawning second scans for small requests on slow cluster days), and (f) pipelines candidate fetching with probing on slimmed candidate columns. Every optimization is default-on with its own config kill switch; rejected tunings from the ablation are documented in the config field descriptions. Verified on 105 production-sampled requests (0 result or page-token diffs) plus new edge-case regression tests.
- **`/addresses/{address}/neighbors` is faster, especially with actor/label info included.** Neighbor lookups now fetch actors, labels and address data in a few grouped database calls per page instead of one round trip per neighbor, with results unchanged.
...
- **The `ghcr.io/graphsense/graphsense-spark` image is published from this repo again**, on `spark-v*` tags. Its workflow did not come across in the move and the first monorepo Spark release shipped without one. The image keeps its own package name rather than following the repository (which would collide with the library image), and the `spark-` prefix is stripped so published tags stay `vYY.MM.P` as before.
- **`full_transform_args.release_tag_prefix`** selects which release track a jar version resolves within. The defaults now point at this repo's `spark-v*` track (`repo: graphsense/graphsense-lib`, `release_tag_prefix: "spark-"`), so an unpinned setup resolves the newest jar built from `spark/`. Jars for `v26.08.0` and earlier were published by the standalone `graphsense-spark` repo, which is archived rather than deleted so those assets keep resolving — pin one by setting `repo` back to `graphsense/graphsense-spark` with an empty prefix. GitHub's "latest release" endpoint is repo-wide, so in a repo publishing several tracks an unprefixed lookup can return a Python-only release that carries no jar. Jar downloads also fall back to the GitHub API when a release's asset is not named after its tag, turning a bare 404 into either a working URL or an error naming the assets that are actually present.
...
#### Added
- **`examples/gs-files/gs_file_roundtrip.py` documents the `.gs` save-file API end to end.** A runnable script that builds a pathfinder spec, checks it with `verify_structural`, lays it out with the standard `apply_hierarchical_layout`, encodes it via `builder_from_spec`, and decodes it back through `decode_gs` / `structure` / `summarize` with a byte-identical round-trip assertion. The graph is real on-chain BTC data, because a `.gs` file is a set of *references* the pathfinder UI resolves against the backend — invented ids produce a file that opens with unconnected nodes, which the inline comments call out along with the tx-must-be-in-both-`txs`-and-`tx_ids` join rule. A PEP 723 header pins the library version, so `uv run --script examples/gs-files/gs_file_roundtrip.py` works from a bare copy of the file with no checkout; the docstring records the per-API minimum versions (`.gs` codec 2.13.0, current layout 2.13.5, warning-clean `verify_structural` 2.15.2) and the example itself runs unmodified on 2.15.1.

- **The cross-chain pubkey dataset now also derives the uncompressed-key address forms for ZEC and BCH.** `utils/pubkey_to_address` emitted only the compressed-key form for Zcash `t1` and Bitcoin-Cash CashAddr (the uncompressed ZEC variant was commented out), while legacy P2PK outputs — which ingest derives network-aware, hashing the key exactly as it appears in the script — overwhelmingly carry *uncompressed* keys. Such an ingested address could therefore never join back to its pubkey on those two chains. `t1_p2pkh_uncomp` and `p2pkh_cashaddr_uncomp` are now derived too, mirroring what the BTC side already did. Affects newly built `pubkey_by_address` data; existing datasets need a rebuild to gain the extra forms. A DOGE P2PK version byte was added alongside for consistency between the two ingest code paths (DOGE is not ingested at the moment), and a new `tests/ingest/test_pubkey_address_version_parity.py` pins the three separate version-byte tables (`rpc_utxo._PUBKEY_ADDRESS_VERSION`, `utxo._NETWORK_SCRIPT_PARAMS`, `pubkey_to_address.MAINNET_ADDRESS_SPECS`) against each other so they cannot drift apart silently.
...
#### Changed
- **Pulling a new Docker image after a code-only release now downloads a few MB instead of ~500 MB.** The runtime stage installs the locked third-party dependency tree (an `uv export` of `uv.lock`, computed in the builder stage) as its own layer, and the two locally-built wheels go in afterwards with `--no-deps` — so the big site-packages layer's digest only changes when the lockfile changes, not on every release. The `uv` helper image and the Temurin 11 base are pinned (version tag / digest) in named build stages: their moving tags invalidated every subsequent layer, re-shipping unchanged apt/Java/dependency layers to prod even for no-op rebuilds, and `FROM` lines are what Dependabot can bump (it ignores images referenced from `COPY --from`). Ownership `chown`s were folded into the layers that create the files, since a standalone `chown -R` duplicates everything it touches into a fresh layer. Dependabot now also watches the `docker` and `github-actions` ecosystems, so those pins and the SHA-pinned workflow actions get weekly grouped bump PRs.
- **The Docker image's third-party dependencies are now exactly the versions in `uv.lock`.** Previously the image resolved `graphsense-lib[all,transformation]` against PyPI at build time, so it shipped whatever latest versions happened to satisfy the ranges — a set nothing had ever tested together (an image built on 2026-07-14, for instance, ran `protobuf` 7.35.1 while CI and the lockfile were on 6.33.6). The image now matches the tested environment, and dependency updates reach it through the usual `uv lock` / Dependabot flow. `gunicorn` in particular is lockfile-pinned via the `web` extra instead of installed floating. Two build-time guards protect the `--no-deps` install of the local wheels: `uv pip check` for base requirements, plus an import assert for the optional CLI groups (`tagpack`, `web`, `transformation`, `mcp`), which `cli/main.py` imports under `try/except ImportError` and would otherwise drop silently if an extra's dependency went missing.
- **The baked Spark-executor environment (`/opt/graphsense/spark-env.tar.gz`) now carries the same locked versions as the driver.** It is built by its own `uv pip install --target` of the crypto stack the Python UDFs need, which until now resolved freely against PyPI on build day — so the executors could run different versions than the driver process submitting the job. In the last image built that way the executors got `eth-utils`/`eth-typing` 6.0.0 against the driver's locked 5.3.1/5.2.1, a major-version skew across the UDF boundary, plus smaller skews in `annotated-types`, `bitarray`, `ckzg`, `regex` and `typing-extensions`. That install is now constrained to the `uv.lock` resolution, and a build-time assert compares every package in the archive against the image's installed version and fails on any divergence.
...
#### Fixed
- **The `graphsenselib.pathfinder` verifiers no longer break on the documented string shorthand.** `builder_from_spec` documents (and `apply_hierarchical_layout` honours) a bare-id form for `addresses` / `txs` — `"txs": ["e67a0550…"]` instead of `[{"id": "e67a0550…"}]` — but both verifiers assumed every entry was a dict. `verify_structural` guarded with `"id" in item`, which on a `str` is a *substring* test rather than a key test, so shorthand entries silently vanished from the id sets: every edge then looked like it referenced txs/addresses "not in `txs`/`addresses`", while genuine orphan-tx and duplicate-id problems went undetected. `verify_against_backend` was worse — a shorthand entry reached `_network_of` as a `str` and raised `AttributeError: 'str' object has no attribute 'get'`, crashing the verify of a spec the encoder accepts. Both now normalize entries up front. The MCP `build_pathfinder_file` path was never affected (it always builds dicts from its pydantic models).

- **Delta-updater `--parallel-workers` no longer masks worker database errors as `TypeError: cannot pickle '_thread.RLock' object`.** When a query in a `ParallelDbPool` worker process failed terminally (retries exhausted), the driver exception — e.g. `NoHostAvailable`, whose `errors` dict is keyed by `cassandra.pool.Host` objects, each holding an `RLock` — could not be pickled back to the parent, so the executor replaced it with the opaque pickling `TypeError` and the real failure was lost. Worker tasks now run through a guard that re-raises pickle-safe exceptions unchanged and converts unpicklable ones into a `ParallelWorkerError` carrying the original exception's formatted traceback.
- **Cassandra username/password auth is now configurable on every code path.** Three gaps would have surfaced when switching a cluster to user/password auth. (1) `tagpack`'s `--use-gs-lib-config-env` read `~/.graphsense.yaml` with a raw `yaml.safe_load`, bypassing the config module: `${VAR}` placeholders were **not** resolved, so a password supplied via environment variable reached Cassandra as the literal string `"${VAR}"`, and `GRAPHSENSE_CONFIG_YAML` / `.graphsense.yaml` file overrides were ignored. It now goes through `get_config()` like every other path. (2) `DbFactory.from_config(readonly=True)` replaced the credentials with `readonly_username`/`readonly_password` unconditionally, so an environment that configures only `username`/`password` would have connected anonymously; the read-write pair is now the fallback, taken atomically so a readonly user is never sent with the read-write password. Shared as `Environment.get_cassandra_credentials()`. (3) `tagpack`'s `GraphSense` client passed `cassandra_nodes` straight to the driver with no port handling, so the `host:port` entries accepted everywhere else silently resolved to a bad host and the port was never applied.
- **`cassandra_nodes` `host:port` parsing is shared rather than reimplemented four times.** `utils/cassandra.split_nodes_and_port` (dependency-free, so both the ingest hot path and the Spark session builder can use it) replaces the copies in `db/cassandra.py`, `tagpack/graphsense.py`, `transformation/spark.py` and `transformation/cli.py`. All callers now warn on conflicting ports instead of silently dropping them, and the bulk-transform pre-flight keyspace check uses every configured node as a contact point instead of only the first.
- **Listing address/entity transactions by `min_date` no longer includes transactions timestamped before `min_date` (issue #52).** `get_min_max_height` translated the date window into an inclusive block-height range but set the lower bound to `before_block` — the last block with timestamp ≤ `min_date` — so a whole block of transactions dated *before* `min_date` leaked into the result (the last row of a `desc` page fell before the requested start). The lower bound is now the first block with timestamp ≥ `min_date` (`before_block` only when it sits exactly on the boundary, otherwise `after_block`), with `min_date` past the chain tip yielding an empty range. The `max_date` bound is unchanged (still inclusive via `before_block`).
- **Sporadic `CRC mismatch on header …` failures are gone: both Cassandra connections (synchronous and asynchronous/REST-serving) now pin native protocol v4.** The error is a client-side segment-framing bug in the Python driver (upstream, unresolved: [CASSANDRA-19971](https://issues.apache.org/jira/browse/CASSANDRA-19971) / PYTHON-1337) triggered under load — not data corruption on the node. Checksummed segment framing exists *only* on protocol v5, which the driver negotiated by default, so pinning v4 removes the failure at the source; v4 still compresses (lz4 at the frame level). Nothing needs v5 — the one v5-ism relied on, statement-level keyspace, is handled version-independently in `CassandraDb.execute` (2.15.0). The earlier 2.15.0 attempt to retry on `CrcException` never fired: that exception is raised on the driver's reactor thread and never reaches the caller — the driver defuncts the connection and callers see a `ConnectionShutdown` carrying the CRC text (failing every in-flight request at once). `TRANSIENT_DB_ERRORS` now lists `ConnectionShutdown` instead, so genuine defuncts (heartbeat loss, node restart) are ridden out by `_execute_with_backoff` too.

#### Security
- **Dependency bumps closing two advisories.** `pydantic-settings` to 2.14.2 (GHSA-4xgf-cpjx-pc3j) and the `ecdsa` floor from `>=0.13` to `>=0.19.2` (CVE-2026-33936); `mcp` moved to 1.28.1 in the same pass. The `ecdsa` floor only became raisable once `chainside-btcpy` and its `ecdsa==0.13` pin were dropped in 2.15.0; `ecdsa` is the `fast=False` fallback behind coincurve in pubkey validation.

### Web API + Python client

#### Added
- **`examples/gs-files/gs_file_roundtrip_client.py` is the client-side counterpart of the new graphsense-lib `.gs` example.** Same real BTC graph, same encode/layout/decode flow through the vendored `graphsense.gs_files`, and byte-identical output — verified against `graphsense-python` 2.15.0. It has no spec-checking step, since `graphsenselib.pathfinder` (`verify_structural` / `verify_against_backend`) is not vendored into the client. A PEP 723 header pins the client, so `uv run --script` runs it without a checkout.

#### Changed
- **The README now documents that the REST YAML config takes precedence over the `GS_CASSANDRA_ASYNC_*` / `GSREST_*` environment variables.** The variables are a per-key *fallback*, not an override — the reverse of the usual convention, and the reverse of how the CLI config (`.graphsense.yaml`, `GRAPHSENSE_*`) resolves the same conflict. Setting `GS_CASSANDRA_ASYNC_PASSWORD` therefore has no effect on a deployment whose config file already pins `database.password`; referencing `${CASSANDRA_PASSWORD}` from the file is the way to keep a secret in the environment. Docs only; no behavior change.
- **The neighbor endpoints now document which response fields are edge-scoped vs. the neighbor's own lifetime attributes (issue #58).** On `GET /{network}/{addresses,entities,clusters}/{id}/neighbors`, `value` and `no_txs` describe the edge between the queried node and the neighbor, while the embedded address/entity object carries the neighbor's own lifetime data (its `first_tx`/`last_tx`, balance, degrees, …) — a distinction that previously had to be inferred. The OpenAPI schema now carries descriptions on those fields and on `Address`/`Entity` `first_tx`/`last_tx` (first/last transaction over the whole history, independent of any neighbor/direction/date filter), and the neighbor endpoint descriptions spell out the edge-vs-lifetime split. Descriptions only; no wire-format change.

#### Fixed
- **Entity ids outside the representable range are treated as absent instead of returning a 500.** Public entity ids exist only in `[0, 2**31)` (legacy, int32-bound in Cassandra and in the tagstore's `int4` mapping columns) or `[2**33, 2**33 + 2**31)` (the fresh-cluster offset plus an int32-bound raw id). Ids in the gap or beyond reached the databases unchecked and blew up on the bind: an asyncpg `DataError` on `/clusters/{id}/tags`, a cassandra `struct.error` on `/entities/{id}` and its subresources. A new `is_representable_entity_id` guard at the data layer makes every route reproduce its existing absent-id behaviour (404 where an absent id 404s, an empty result where an absent id is empty), without issuing a query.
- **Bulk `get_rates` no longer truncates its response mid-stream.** The service returned internal pydantic `Rate` models nested inside a plain dict; `flatten(format="json")` passes dicts through untouched, so `json.dumps` raised `TypeError: Object of type Rate is not JSON serializable` *after* the 200 header had been sent — `bulk.json`/`get_rates` clients received a truncated body. The models are now dumped at the service boundary.
- **Date-filtered queries on a network whose chain does not start at block 0 no longer 500 (TRX).** The binary search in `get_block_by_date` assumed every height in `[0, no_blocks - 1]` has a row in the raw block table, but TRX ingest starts at block 1. A `min_date`/`max_date` below the first stored block's timestamp made the search probe height 0, `get_block_timestamp` returned `None`, and the comparison raised an unhandled `TypeError`. The search's lower bound is now clamped to the first stored block. (While there, an `alru_cache` on the search was dropped: its key included the per-call `get_timestamp` closure, so every lookup missed and the cache only accumulated dead entries — block timestamps are already cached in the db layer.)
- **The links-timeout error on ETH/TRX now names the addresses in their canonical form** instead of printing the raw byte representation.
...
#### Added
- **Fresh UTXO clustering: a PySpark one-off bootstrap plus incremental clustering in the delta updater.** `transformation cluster` clusters a whole chain in one run (streaming into an in-process Rust union-find) and writes the new `fresh_*` tables. The delta updater then keeps them current per batch. Cluster ids are canonical `min(address_id)`, and only multi-member clusters are stored (a missing address is its own singleton). Coinjoin co-spends are excluded, matching the legacy Scala transform. Cluster statistics are updated with every batch (`recompute-cluster-stats` remains as an optional backstop), and the fresh writes commit together with the rest of the delta batch, so a crash replay cannot double-count them. **Deployments already running fresh clustering from pre-release builds must run the stats recompute once at upgrade.**
- **Fresh clusters are served through self-describing entity ids.** Entity ids at or above 2^33 route to the fresh tables, ids below to the legacy `cluster` table. The shift is a REST-boundary translation only, so no data migration is needed. Activation is a per-keyspace marker written by the one-off bootstrap as its final step and requires no REST configuration change or restart. Once the marker is present, address responses carry a `fresh_cluster_id`. Singletons are synthesized on read, while degrees and cluster relations are still served via the legacy cluster.
- **`GRAPHSENSE_TAGSTORE_FRESH_CLUSTERS` flips the tagpack cluster-mapping feeder to the parallel `*_v2` relations**, reading membership from the fresh tables. Cluster-tag reads route on the public id, so inherited cluster tags resolve via the fresh id once active. The first v2 fill needs a full `--rerun-cluster-mapping-with-env`.
- **`deltaupdate --parallel-workers` now also covers the UTXO updater (v2).** Batch reads and writes fan out to the worker pool, and it composes with in-loop fresh clustering and the WAL. `TX` application mode stays single-process.
- **New `transformation top-untagged-addresses` CLI command (ALPHA) ranks the most active addresses that carry no tag yet.** A PySpark job reads the transformed `address` table, ranks by `--sort-by txs|value|fiat|degree` (`--fiat-currency USD|EUR`), removes every address the tagstore already knows, and writes the top `--limit` rows to CSV or Parquet (`--out-path`, `--format`). A local path becomes a single driver-written file, an `s3://` path a Spark-written directory of part files. Every row carries all four activity metrics. On UTXO networks it also carries a `cluster_tagged` flag, set when the address itself is untagged but its cluster has a tag (null on account-model networks). To stay off the full ~1e9-row address table, the job ranks a candidate pool of `--limit × --candidate-multiplier` (default 50) before tag-filtering, warning when tagged addresses exhaust the pool. The tagstore is probed with chunked indexed lookups rather than scanned (psycopg2 via `--tagstore-db-url`, default `GS_TAGSTORE_DB_URL`, plus `--tagstore-schema`), and each run logs the candidate pool's tag coverage.
- **New `convert scan-for-addresses` CLI command scans text/SQL files (and compressed containers) for cryptocurrency addresses.** Extraction is deliberately permissive. The real filter is checksum validation (reusing `utils/address.py:validate_address`), which drops the many address-shaped false positives in DB dumps (hashes, session ids, base64 blobs). Validates BTC (legacy/bech32), LTC (legacy/bech32), ETH (EIP-55), TRX, ZEC (transparent) and XRP, and reports XMR as candidates only. Compressed inputs (gzip/zlib/bz2/xz/zip/tar, recursively, plus GraphSense `.gs` files) are unwrapped transparently, with `--carve` to inflate zlib/gzip streams embedded in binaries and a decompression-bomb budget. Opt-in `--tx-hashes` reports 64-hex tx-hash *candidates* (format-only, not checksum-verifiable, so the command warns that SHA-256 digests/tokens are picked up too). Supports `--json`, `--context`, and rejected-candidate listing. The `convert` command group is now also reachable as `file` (e.g. `graphsense-cli file scan-for-addresses ...`). Adds `validate_xrp_address` (and `xrp`) to `utils/address.py:validate_address`.
...
- **The UTXO delta updater nets flows per (tx, address) before deriving relations and statistics, matching the Spark transform.** An address on both sides of a tx was counted in both directions, creating relation and stats rows Spark never writes. `/links` deliberately keeps unnetted semantics, since an address taking change still genuinely received an output. Keyspaces updated by earlier versions keep the extra rows until a re-transform.
- **Tagpack cluster-mapping no longer aborts on a dangling `cluster_id` reference** (a cluster whose stats row is not yet written). The unknown cluster size is stored as NULL rather than a fabricated 1, which would misroute a real cluster's tags.
- **Resuming an append ingest after a mid-chunk crash no longer produces duplicate Delta rows.** Orphaned side-table rows left by the crashed chunk are removed before the resume.
- **Address responses no longer 500 on zero-tx address rows** (`first_tx_id = -1`, written for fee-only senders and miner rewards): `first_tx`/`last_tx` are optional and the sentinel is guarded.
...
- **The async read path no longer blocks the event loop on Cassandra reconnect**, and the batch read helpers now ride out transient stalls with the same backoff as synchronous reads.
- **Search no longer mishandles unicode ligatures** (PDF copy-paste can render `ff` as the single character `ﬀ`). Search input is folded back to plain ASCII before validation.
...
- **The batch trace exporter no longer retries `trace_block` up to 225 times against a dead endpoint.** `_fetch_traces_for_blocks` wrapped `make_batch_request` (which already retries transient failures 15× with exponential backoff and session reset) in a second 15× retry loop, nesting to 15×15 attempts and hammering an unreachable node for over an hour before failing. The redundant outer loop is removed.
