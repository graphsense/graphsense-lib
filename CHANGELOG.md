...
#### Added
- **New `transformation top-untagged-addresses` CLI command (ALPHA) ranks the most active addresses that carry no tag yet.** A PySpark job that reads the transformed keyspace `address` table from Cassandra, removes every address the tagstore already has a tag for, and writes the top `--limit` remaining addresses to CSV or Parquet (`--out-path`, `--format`) — the intended input for prioritising tagging work. The result is at most `--limit` rows, so a local `--out-path` is collected to the driver and written as a single file, named after its format if the path has no extension (`/out/btc-untagged` → `/out/btc-untagged.csv`); only `s3://`-style paths go through Spark's distributed writer, and those are used verbatim (Spark writes a *directory* of part files there) (a scheme-less path handed to that writer resolves as `file:` on whichever *executor* runs the write task, which fails with `Mkdirs failed to create` unless the path happens to exist cluster-wide). Every row carries all four activity metrics (`no_txs` = `no_incoming_txs + no_outgoing_txs`, `degree` = `in_degree + out_degree`, `total_received_value` in the chain's native smallest unit, and `total_received_fiat`); `--sort-by txs|value|fiat|degree` picks which one ranks, `--fiat-currency USD|EUR` picks the fiat column. For UTXO networks each row also carries a `cluster_tagged` flag: the address itself is untagged, but its cluster is — a strong hint that it belongs to a known entity (null for account-model networks, which have no clustering). Address exclusion matches `tag.identifier` network-agnostically (a tag on any network means the address is not unknown), while `cluster_tagged` is network-scoped, since `gs_cluster_id` is only meaningful within one network. To keep the work off the full ~1e9-row address table, the job ranks a candidate pool of `--limit × --candidate-multiplier` (default 50) addresses *before* removing tagged ones, and warns when the pool is exhausted by tagged addresses so fewer than `--limit` rows come out. The tagstore is **probed, not scanned**: the candidate pool is collected to the driver and checked with chunked `identifier = ANY(...)` queries riding the `ix_tag_identifier` btree (and `best_cluster_tag(network, cluster_id)` for the cluster flag), so a run costs a few tens of thousands of index probes rather than a full pass over `tag`, which would ship every identifier into the Spark executor to compare against a candidate pool orders of magnitude smaller. Connects with psycopg2 (already a base dependency, so no JDBC driver is added to the Spark packages) via `--tagstore-db-url`, defaulting to the `GS_TAGSTORE_DB_URL` setting, and `--tagstore-schema`. Each run logs the tag coverage of its candidate pool — how many of the top-ranked addresses (and, for UTXO, of their clusters) the tagstore already knows — reusing the probe results, so the numbers cost no extra query. Note this is coverage *of the most active addresses*, not of the chain.
...
#### Fixed
- **`transformation top-untagged-addresses` ranked the wrong addresses on account chains (eth, trx); UTXO chains were unaffected.** The job rendered the account `address` blob to its `0x…`/base58 identifier with a PySpark Python UDF, appended to the `orderBy(...).limit(n)` that selects the candidate pool. Appending that UDF projection made Catalyst re-plan the limit: measured against `eth_transformed`, the identical pool went from `[1.235e22 … 7.82e26]` wei to `[6.84e19 … 1.001e21]` — a different 50 000 rows, capped three orders of magnitude too low, with the exact `Decimal(38,0)` sort key degraded to float64 (`1001e18` came back as `1000999999999999967232`). The job then tag-filtered and ranked that wrong pool correctly, so the output was monotonic, dense and superficially plausible while consisting of arbitrary mid-tier addresses. UTXO keyspaces store `address text` and never attached the UDF, so their output was always correct. Addresses are now rendered on the driver — where the candidate pool already lands to probe the tagstore — and joined back via a broadcast lookup, so the job runs no Python on the executors at all. **Any eth/trx output produced by an earlier build must be discarded and regenerated.**
...
- **Sporadic `CRC mismatch on header …` failures are gone: both Cassandra connections (synchronous and asynchronous/REST-serving) now pin native protocol v4.** The error is a client-side segment-framing failure in the Python driver (upstream, unresolved: [CASSANDRA-19971](https://issues.apache.org/jira/browse/CASSANDRA-19971) / PYTHON-1337) triggered under load — not data corruption on the node. Checksummed segment framing exists *only* on protocol v5, which the driver picked whenever the version was left to negotiate, so pinning v4 removes the failure mode at the source rather than retrying around it. Note that **disabling compression is not an equivalent fix**: the header CRC24 is verified over the header bytes alone, *before* any decompression (the payload CRC32 is likewise checked pre-decompress), so a *header* mismatch means the driver read a segment header at the wrong offset — a buffer desync in framing code that is shared verbatim by the compressed and uncompressed segment codecs. Dropping compression would keep that code path and forfeit the bandwidth saving, whereas v4 keeps lz4 (applied at frame level on pre-v5 protocols) and drops the framing. Nothing needs v5: the one v5-ism relied upon, statement-level keyspace, is already handled version-independently in `CassandraDb.execute` (see the entry below). A previous attempt to ride the error out by adding `CrcException` to `TRANSIENT_DB_ERRORS` never fired — that exception is raised on the driver's I/O reactor thread and is never delivered to the caller: the driver defuncts the connection and `error_all_requests()` re-wraps the cause, so callers see a **`ConnectionShutdown`** carrying the CRC text, and *every* in-flight request on that connection fails at once. Hence the crash it was meant to prevent (`ConnectionShutdown: CRC mismatch on header …` killing a `delta-update -c trx` run). `TRANSIENT_DB_ERRORS` now lists `ConnectionShutdown` instead, so genuine connection defuncts (heartbeat loss, node restart) are also ridden out by `CassandraDb._execute_with_backoff` rather than surfacing to callers. The **asynchronous** driver (REST serving path) had explicitly pinned v5 and was equally exposed — a CRC event there fails every in-flight request on the connection, i.e. a burst of 500s rather than one; it is pinned to v4 as well. It never needed v5 either: its queries carry the keyspace in the CQL itself (`replaceFrom`), not in the v5-only statement-level keyspace flag.
