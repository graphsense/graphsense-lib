...
- **The Address schema declares a flat `qualifiers` map** (field name → `"gt"` for lower-bound values, `"approx"` for sample-approximations) as the simple consumer form of the existing `cutoff` qualification. Set only by external backends with provider-call budgets; local serving computes exact aggregates and never emits it.
