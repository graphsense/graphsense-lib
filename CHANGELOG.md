...
#### Added
- **Async Cassandra reader can downgrade LOCAL_QUORUM to LOCAL_ONE on transient unavailability.** New `CassandraConfig.consistency_level_fallback` (default `false`, opt-in). When enabled, the read-path retry policy (`GraphsenseFallbackToLocalOneRetryPolicy`, in `db/asynchronous/cassandra.py`) downgrades a LOCAL_QUORUM read to LOCAL_ONE on the FIRST `Unavailable` / `ReadTimeout` if at least one replica is alive / responding — letting the web tier survive a rolling restart on RF=2 (where losing one of two replicas takes LOCAL_QUORUM below quorum). Strictly scoped: only one downgrade per query (subsequent retries delegate to the parent retry/backoff machinery), only LOCAL_QUORUM is touched (QUORUM / ALL / EACH_QUORUM are deliberate trade-offs and left alone), and writes are never downgraded. Trade-off: read-after-write consistency is no longer guaranteed for the downgraded reads — opt in only when LOCAL_QUORUM availability matters more than that guarantee.
...
- **Backend HTTP 5xx errors now fire Slack via the MCP path.** `mcp/tools/consolidated.py`'s `_get_json` / `_get_json_optional` previously wrapped every non-2xx upstream response (including 5xx) as a `ToolError`, which the middleware above passes through silently — so backend crashes were invisible to ops. A new helper `_raise_backend_http_error` now logs at `ERROR` on `graphsenselib.mcp.tools.consolidated` (propagating to Slack) when the upstream status is `>= 500`, then raises the same `ToolError` so the model's retry contract is unchanged. 4xx stays silent — those are model-fixable inputs (`tx not found`, `bad address`), not incidents.
- **MCP call-argument validation errors no longer fire Slack.** Observed in production: a Mistral agent nested the top-level `layout` argument inside `spec`, FastMCP rejected the call with a Pydantic `extra_forbidden` validation error, and `ErrorLoggingMiddleware` treated that as an incident and paged ops. Caller-side validation errors are the spiritual equivalent of a 4xx — model-fixable and worth a retry, not an incident. `fastmcp.exceptions.ValidationError` and `pydantic.ValidationError` are now in `_EXPECTED_MCP_ERRORS` and pass through silently alongside `ToolError`, `ResourceError`, `PromptError`. The model still receives the error response and can correct its call.
- **`build_pathfinder_file` accepts `layout` nested inside `spec` as a forgiving alias.** LLMs frequently put `layout` inside `spec` because it reads as a graph-shape option. Previously this produced a Pydantic `extra_forbidden` validation error the model couldn't recover from. `PathfinderSpec` now declares an optional `layout` field; when the top-level argument is the default (`"auto"`) and `spec.layout` is set, the latter wins. An explicit top-level `layout` still takes precedence.
- **`build_pathfinder_file` always emits a TextContent block alongside the embedded resource / download link.** Observed on Mistral Le Chat: with `file_store.embed_resource=false` the tool returned `content=[]` and only put the download URL into `structured_content`. Le Chat (and other weak MCP hosts) only render `content` in the chat UI and ignore `structured_content` — so the user saw nothing despite the call succeeding. The tool now always appends a `TextContent` line carrying the download URL (or "file is embedded in this response" when no link is available); the embedded resource and structured content are unchanged.
