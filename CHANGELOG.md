...
- **Cluster label lookups no longer bleed across networks.** `get_labels_by_clusterid` had no network predicate, so a numeric cluster id matched rows on every chain (e.g. an LTC label surfacing on a BTC entity) and could not use the `(network, gs_cluster_id)` index. It now filters by network. Label search additionally escapes `%`/`_` wildcards in user input and paginates with a deterministic order.
