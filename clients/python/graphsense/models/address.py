        ...
        if self.token_balances:
            for _key_token_balances in self.token_balances:
                if self.token_balances[_key_token_balances]:
                    _field_dict[_key_token_balances] = self.token_balances[
                        _key_token_balances
                    ].to_dict()
        ...
        if self.total_tokens_received:
            for _key_total_tokens_received in self.total_tokens_received:
                if self.total_tokens_received[_key_total_tokens_received]:
                    _field_dict[_key_total_tokens_received] = (
                        self.total_tokens_received[_key_total_tokens_received].to_dict()
                    )
        ...
        if self.total_tokens_spent:
            for _key_total_tokens_spent in self.total_tokens_spent:
                if self.total_tokens_spent[_key_total_tokens_spent]:
                    _field_dict[_key_total_tokens_spent] = self.total_tokens_spent[
                        _key_total_tokens_spent
                    ].to_dict()
        ...
        _obj = cls.model_validate(
            {
                ...
                "balance": Values.from_dict(obj["balance"])
                if obj.get("balance") is not None
                else None,
                "total_received": Values.from_dict(obj["total_received"])
                if obj.get("total_received") is not None
                else None,
                "total_spent": Values.from_dict(obj["total_spent"])
                if obj.get("total_spent") is not None
                else None,
                "first_tx": TxSummary.from_dict(obj["first_tx"])
                if obj.get("first_tx") is not None
                else None,
                "last_tx": TxSummary.from_dict(obj["last_tx"])
                if obj.get("last_tx") is not None
                else None,
                ...
                "actors": [LabeledItemRef.from_dict(_item) for _item in obj["actors"]]
                if obj.get("actors") is not None
                else None,
